...
const BottomPanel: React.FC<BottomPanelProps> = ({
  ...
  const tabButtonClasses = (tabName: 'problems' | 'terminal') => {
    const base = 'flex items-center gap-2 px-4 py-2 text-sm font-medium focus:outline-none transition-all duration-200';
  ...
  const renderTerminal = () => (
    <div className="h-full flex flex-col p-4" onClick={() => commandInputRef.current?.focus()}>
        <pre ref={terminalOutputRef} className="flex-grow whitespace-pre-wrap break-words font-mono text-sm text-slate-400">{terminalOutput}</pre>
        <form onSubmit={handleCommandSubmit} className="flex items-center gap-2 mt-2">
            ...
            <input
                ...
                placeholder="Type a command... (npm packages are supported)"
